...
QString FileBrowserModel::uint64ToPercent(quint64 part, qreal whole, quint32 precision)
{
    ...
    qreal shift {1};
    for (quint32 i {0}; i < precision; i++) shift *= 10;
...
QString FileBrowserModel::uint64ToSmart(quint64 size)
{
    ...
    while (newSize >= 1024) { newSize /= 1024; index++; }
    if (index > -1 && index < 6)
...
{
    quint64 whole {0};
    foreach (const QString& key, sizesMap.keys()) whole += sizesMap.value(key);
    beginResetModel();
    entrances.clear();
    foreach (const QString& key, sizesMap.keys())
        entrances.append(QList<QString> { key, uint64ToSmart(sizesMap.value(key)), uint64ToPercent(sizesMap.value(key), whole) });
    endResetModel();
}


int FileBrowserModel::rowCount(const QModelIndex& parent) const { Q_UNUSED(parent); return entrances.size(); }
int FileBrowserModel::columnCount(const QModelIndex& parent) const { Q_UNUSED(parent); return HEADER.size(); }
